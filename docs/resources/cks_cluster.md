...
```terraform
resource "coreweave_cks_cluster" "ipv6" {
  name                   = "ipv6-cluster"
  version                = "v1.35"
  zone                   = "US-EAST-04A"
  vpc_id                 = coreweave_networking_vpc.default.id
  ...
  # IPv6 configuration (all IPv6 fields must be set together)
...
- `pod_cidr_name_v6` (String) The name of the VPC prefix to use as the IPv6 pod CIDR range.  
  If any IPv6 field is set, all IPv6 fields must be set. This field is immutable after cluster creation.
- `service_cidr_name_v6` (String) The name of the VPC prefix to use as the IPv6 service CIDR range.  
  If any IPv6 field is set, all IPv6 fields must be set. This field is immutable after cluster creation.
- `internal_lb_cidr_names_v6` (Set of String) The names of the VPC prefixes to use as IPv6 internal load balancer CIDR ranges.  
  If any IPv6 field is set, all IPv6 fields must be set. These prefixes must exist in the cluster's VPC.
...
IPv6 support is optional and must be configured at cluster creation time.

If any IPv6 network field is specified, all IPv6 network fields must be specified.  
IPv6 network fields cannot be added to an existing cluster after creation.
...
- `ca` (String) The CA certificate for the authorization webhook. Must be a base64-encoded PEM-encoded certificate.
