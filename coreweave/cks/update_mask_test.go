...
func TestBuildUpdateRequest(t *testing.T) {
	...
	t.Run("network change only populates network field", func(t *testing.T) {
		...
		expectMaskPaths(t, req.UpdateMask.Paths,
			"network.internal_lb_cidr_names",
			"network.service_node_port_range.start",
			"network.service_node_port_range.end",
		)
	})
	...
	t.Run("version and network change together", func(t *testing.T) {
		...
		req := buildUpdateRequest(ctx, &plan, &state)
		if req.Version != testUpgradeVersion {
			t.Errorf("expected version %s, got %s", testUpgradeVersion, req.Version)
		}
		if req.Network == nil {
			t.Fatal("expected non-nil network")
		}
	...
	t.Run("oidc added", func(t *testing.T) {
		...
		expectMaskPaths(t, req.UpdateMask.Paths,
			"oidc.issuer_url", "oidc.client_id", "oidc.username_claim",
			"oidc.username_prefix", "oidc.groups_claim", "oidc.groups_prefix",
			"oidc.ca", "oidc.required_claim", "oidc.signing_algorithms",
			"oidc.admin_group_binding",
		)
	})
	...
	t.Run("authn_webhook added", func(t *testing.T) {
		...
		expectMaskPaths(t, req.UpdateMask.Paths, "authn_webhook.server", "authn_webhook.ca")
	})
	...
	t.Run("authz_webhook added", func(t *testing.T) {
		...
		expectMaskPaths(t, req.UpdateMask.Paths, "authz_webhook.server", "authz_webhook.ca")
	})
