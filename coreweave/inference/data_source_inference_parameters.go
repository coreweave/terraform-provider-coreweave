...
func (d *InferenceParametersDataSource) Read(ctx context.Context, _ datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// instance_types
	itVals := make([]attr.Value, 0)
	if rp := msg.GetResourceParameters(); rp != nil {
		for _, it := range rp.GetInstanceTypes() {
			itVals = append(itVals, types.StringValue(it))
		}
	}
	data.InstanceTypes = types.ListValueMust(types.StringType, itVals)
